...
src.add_argument("-p", "--port", metavar="Port",
                 help="You can type one port or more like p1,p2,p3... "
                      "if you want to give range you have to type like max_port min_port ",
                 type=str, nargs="+")
...
class PortStatus:
    ...
    def parse_and_print(self, show_closed=True, show_open=True):
        ...
        for port, status, service in sorted(self.PORT_LIST, key=lambda x: x[1] == "open" or x[0]):
            self.screen.addstr(" " * 5 + str(port) + " " * (16 - len(str(port)) - len(str(status))) + status + " " * 10 + service)
        self.screen.addstr("\n\nScanning completed in {0:.2f} seconds".format(self.TIME))
        self.screen.refresh()
        if args.write:
            try:
                f = open(
                    os.curdir + "/output/" + args.write if os.path.basename(args.write) == args.write else args.write,
                    "w", encoding="utf-8")
                if args.write.endswith(".txt"):
                    ...
                    json.dump([{"port": port, "status": status, "service": service} for port, status, service in
                               sorted(self.PORT_LIST, key=lambda x: x[1] == "open" or x[0])], f)
...
def main():
    scr = curses.initscr()
    scr.addstr(pyfiglet.figlet_format("PortScanner V1")+"\n")
    scr.refresh()
    if args.get:
        ...
        try:
            with open(os.curdir + "/input/" + args.get if os.path.basename(args.get) == args.get else args.get,
                      "r") as file:
